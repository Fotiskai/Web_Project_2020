...
for($i=0;$i<13;$i++){
	...
	$sql="SELECT COUNT(DISTINCT(id)) as vehicle FROM activities WHERE userid='$uid' AND timestampMs <= LAST_DAY(DATE_SUB(CURDATE(), INTERVAL $tmp MONTH)) AND timestampMs > DATE_ADD(DATE_ADD(LAST_DAY(DATE_SUB(CURDATE(), INTERVAL $tmp MONTH)),interval 1 DAY),interval -1 MONTH) AND type IN('IN_VEHICLE','IN_RAIL_VEHICLE','IN_ROAD_VEHICLE','IN_FOUR_WHEELER_VEHICLE','IN_CAR','IN_TWO_WHEELER_VEHICLE','IN_BUS')";
	$result=mysqli_query($conn, $sql);
	$veh_c=mysqli_fetch_assoc($result);	
	$sql="SELECT COUNT(DISTINCT(id)) as walk FROM activities WHERE userid='$uid' AND timestampMs <= LAST_DAY(DATE_SUB(CURDATE(), INTERVAL $tmp MONTH)) AND timestampMs > DATE_ADD(DATE_ADD(LAST_DAY(DATE_SUB(CURDATE(), INTERVAL $tmp MONTH)),interval 1 DAY),interval -1 MONTH) AND type IN('ON_BICYCLE','ON_FOOT','WALKING','RUNNING')";
	$result=mysqli_query($conn, $sql);
...
$sql="SELECT MIN(timestampMs) as minn, MAX(timestampMs) as maxn FROM activities WHERE userid='$uid'";
$result = mysqli_query($conn, $sql);
$res=mysqli_fetch_assoc($result);
